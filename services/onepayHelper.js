...
let onepayConfig = require('../config/payment.json')['onepay'][env];

let paramsKey = ['Title', 'vpc_AccessCode', 'vpc_Amount', 'vpc_Command', 'vpc_Currency', 'vpc_Customer_Id', 'vpc_Customer_Phone', 'vpc_Locale', 'vpc_MerchTxnRef', 'vpc_Merchant', 'vpc_OrderInfo', 'vpc_ReturnURL', 'vpc_SHIP_City', 'vpc_SHIP_Country', 'vpc_SHIP_Provice', 'vpc_SHIP_Street01', 'vpc_TicketNo', 'vpc_Version'];
paramsKey.sort();
...
let defaultObj = {
  Title: 'Tổ Cú',
  vpc_Version: 2,
  vpc_AccessCode: onepayConfig.vpc_AccessCode,
  vpc_Command: 'pay',
  vpc_Currency: 'VND',
  vpc_Locale: 'vn',
  vpc_Merchant: onepayConfig.vpc_Merchant,
  vpc_ReturnURL: onepayConfig.vpc_ReturnURL,
  vpc_MerchTxnRef: randomBytes
};


export function makeUrlRequest(paramsObj) {

  Object.assign(paramsObj, defaultObj);
  ...
  for (let i = 0; i < paramsKey.length; i++) {
    ...
    if (value) {
      if (i === 0) {
        paramsUrl = paramsUrl + encodeURIComponent(key) + '=' + encodeURIComponent(value);
      }
      else {
        paramsUrl = paramsUrl + '&' + encodeURIComponent(key) + '=' + encodeURIComponent(value);
      }

      if (key.substr(0, 4) == 'vpc_' || key.substr(0, 5) == 'user_') {
        stringHashData = stringHashData + key + '=' + value + '&';
      }
    }
  }
  ...
  let vpc_SecureHash = crypto.createHmac("SHA256", pack('H*', onepayConfig.SECURE_SECRET)).update(stringHashData).digest("hex").toUpperCase();

  let url = onepayConfig.vpc_URL + '?' + paramsUrl + '&vpc_SecureHash=' + vpc_SecureHash;

  return url;
}

export function hashValidate(paramsObj) {
  return new promise((resolve, reject) => {
    if(paramsObj['vpc_TxnResponseCode'] == 7 || paramsObj['vpc_TxnResponseCode'] == 'No Value Returned') return reject('INVALID HASH');
    ...
    if(paramsObj['vpc_SecureHash'] == vpc_SecureHash) {
...
function onepayValidate(params) {
  return new promise((resolve, reject) => {
